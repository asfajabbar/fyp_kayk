...
import com.example.project_kayk.Adapters.OrderAdapter;
import com.example.project_kayk.Model.MyorderModel;
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                        ...
                        {
                            products+= "    #"+data.getKey() + "\n        Price: " + data.child("productPrice").getValue().toString() + " Rs\n        Quantity: " + data.child("quantity").getValue().toString()+"\n";
                        }

                        orderItemList.add( new MyorderModel(dataSnapshot.getKey(),"   Date :  " + Date ,"   Products Number :  "+String.valueOf(nums),"   Total Price :  "+ totalPrice+" Rs" , "   "+products,OrderCheck));
                    }
                }
                else{
                    orderItemList.clear();
