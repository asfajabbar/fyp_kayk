...
public class ProductInfoActivity extends AppCompatActivity  implements NavigationView.OnNavigationItemSelectedListener{
        ...
        PIsFav.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(ProductIsFavorite.equalsIgnoreCase("true")){
                    ...
                    x.child("productprice").setValue("Rs "+ProductPrice);
    ...
    private void setProductData(){
        if (SingletonClass.image.equals("")){
            Picasso.get().load(ProductImage).into(PImage);
        }
        else  {
            setImage(SingletonClass.image);
        }
        ...
        if(IsOffered.equalsIgnoreCase("yes")){
            ...
            PPrice.setText("Price: "+PriceAfterOffer+" Rs");
            OldPrice. setText(ProductPrice+" Rs");
            OfferRate.setText("- 30%");
            OldPrice.setPaintFlags(OldPrice.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
        }
        else{
            OfferContainer.setVisibility(View.GONE);
            PPrice.setText("Price: "+ProductPrice+" Rs");
        }
...
//        else {PExpiryDate.setVisibility(View.VISIBLE); PExpiryDate.setText("Expiry Date: "+ProductNExpiryDate);}
        ...
        ValueEventListener valueEventListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    for(DataSnapshot dataSnapshot : snapshot.child("Mini Treats").getChildren()){
                        if(dataSnapshot.getKey().equals(ProductName)){
                            PCategory.setText("Bakery: Mini Treats");
                            PAmount.setText("Available Amounts: "+dataSnapshot.child("quantity").getValue());
                            break;}
                    }
                    for(DataSnapshot dataSnapshot : snapshot.child("Jalal Sons").getChildren()){
                        if(dataSnapshot.getKey().equals(ProductName)){
                            PCategory.setText("Bakery: Jalal Sons");
                            PAmount.setText("Available Amounts: "+dataSnapshot.child("quantity").getValue());
                            break;}
                    }

                    for(DataSnapshot dataSnapshot : snapshot.child("Layers").getChildren()){
                        if(dataSnapshot.getKey().equals(ProductName)){
                            PCategory.setText("Bakery: Layers");
                            PAmount.setText("Available Amounts: "+dataSnapshot.child("quantity").getValue());
                            break;}
                    }

                    for(DataSnapshot dataSnapshot : snapshot.child("Cakes & Bakes").getChildren()){
                        if(dataSnapshot.getKey().equals(ProductName)){
                            PCategory.setText("Bakery: Cakes & Bakes");
                            PAmount.setText("Available Amounts: "+dataSnapshot.child("quantity").getValue());
                            break;}
                    }
